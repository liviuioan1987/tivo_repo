...
class Plex_processor:
    def __init__(self,solution_source_file,plex_partner_id,list_msoservice_IDs,channel_start_range,folder,file_name):
    ...
    def plex_csv_to_data(self):
        ...
        for channel in self.page_row:
            if str(channel[3] )in "10427289051":
                channel[3] = "446201192"
            if "Motorsport.tv" in str(channel[1]):
                channel[1] = "Motorsport.tv"
    ...
    def save_file(self):
        try:
            self.excel_file_path = f"{self.folder}/Plex_US_{self.file_name}_file.xlsx"
