...
class Xumo_processor():
    ...
    def xumo_csv_to_data(self):
        ...
        list_confluence_channels = [{"start_date": "01/01/1970 12:00:01 AM", "end_date": "12/31/2099 11:59:59 PM",
                                     "linear_channel_nr": None, "packaged_service_id": None, "call_sign": entry[4],
                                     "channelName": entry[1], "stationId": "epgProvider:st."+str(entry[3]),
                                     "url": entry[6], "channel_description": entry[5]} for entry in channel_data]
    ...
    def save_file(self):
        try:
            self.excel_file_path = f"{self.folder}/Xumo_US_{self.file_name}_file.xlsx"
