...
class PlutoCA_processor:
    def __init__(self, solution_source_file, pluto_partner_id, list_msoservice_IDs, channel_start_range,packaged_service_id, mso_all_data_file, folder, file_name):
    ...
    def pluto_csv_to_data(self):
        ...
        extracted_data = [[row[1], row[2], row[6], row[8]] for row in open_csv_file if row[0].isdigit()]
        list_confluence_channels = [
            {"start_date": "01/01/1970 12:00:01 AM", "end_date": "12/31/2099 11:59:59 PM", "linear_channel_nr": None,
             "packaged_service_id": None, "channelName": entry[0], "provider_description": entry[1],
             "stationId": "epgProvider:st." + entry[2], "deeplinkParam": entry[3]} for entry in extracted_data]
        ...
        self.list_confluence_channels_stripped = [item for item in list_confluence_channels if
                                                  len(pattern_stationid.findall(item["stationId"])) >= 1]

        """        # Removing the =URL formatting if found at channel level
                for channel in self.list_confluence_channels_stripped:
                    url_search = pattern_stationid.findall(channel["deeplinkParam"])
                    if url_search:
                       channel["deeplinkParam"] = url_search[0]
        ...
        for channel in self.list_confluence_channels_stripped:
            ...
            if "Motorsport.tv" in channel["channelName"]:
                channel["channelName"] = "Motorsport.tv"
    ...
    def open_mso_file_data(self):
        ...
        all_data = data.loc[:,
                   ["Packaged Service Id", "Partner Station Id", "Station Name", "Availability Window Start",
                    "Availability Window End"]]
        y = all_data.query(f'{self.packaged_service_id} <= `Packaged Service Id`  < {self.packaged_service_id + 2000}')
        ...
        self.validation_data = [
            {"channelName": channel_st_id[i], "stationId": prtn_st_id[i], "packaged_service_id": self.pckd_serv_id[i],
             "start_date": start_date[i], "end_date": end_date[i]} for i in range(len(prtn_st_id))]
    ...
    def write_channel_page(self):
        ...
        header_data = ["Packaged Service Id", "Partner Station Id", "Call Sign", "Station Name",
                       "Packaged Service Description", "Jump Channel Type", "Availability Window Start", \
                       "Availability Window End", "Linear Service Type", "DVB Linkage Id", "Linear Provider Partner Id",
                       "Recording Provider Partner Id", \
                       "Partner Channel Id", "Video Resolution", "Prevent EAS Interruption", "Channel Change Id", \
                       "Playback URI Channel Id", "IP ABR URL", "DRM Type", "Transport Encoding Type", "Device Type",
                       "SOCU Base URL", "Application Id", "Channel Name", \
                       "Channel Description", "Guide Cell Title", "Logo Partner Id", "DRM Content ID", "MultiView","Pre-roll Ad"]
        ...
        for entry in self.list_confluence_channels_stripped:
            ...
            channel_sheet.cell(row=incremental_row,
                               column=5).value = f'{entry["provider_description"]}-{entry["channelName"]}'
    ...
    def write_msoSiteID_pages(self):
        ...
        page_columns = ["Packaged Service Id", "Partner Station Id", "Call Sign", "Station Name",
                        "Packaged Service Description", "Partner Channel Id", \
                        "Logical Channel Number", "IP ABR URL", "Channel Change Id", "Playback URI Channel Id",
                        "Preferred Transport", "Device Type", "DRM Type", "Transport Encoding Type", \
                        "SOCU Base URL", "Application Id", "Channel Name", "Channel Description", "Guide Cell Title",
                        "Logo Partner Id" \
                        ]
        ...
        for data in range(len(page_columns)):
            ...
            for entry in self.list_confluence_channels_stripped:
                ...
                channel_sheet.cell(row=incremental_row, column=9).value = entry["deeplinkParam"]
                channel_sheet.cell(row=incremental_row, column=10).value = entry["deeplinkParam"]
    ...
    def save_file(self):
        try:
            self.excel_file_path = f"{self.folder}/Pluto_CA_{self.file_name}_file.xlsx"
