...
class PlexCA_processor:
    def __init__(self,solution_source_file,plex_partner_id,list_msoservice_IDs,channel_start_range,packaged_service_id,mso_all_data_file,folder,file_name):
        self.workbook = openpyxl.Workbook()
        self.solution_source_file = solution_source_file
        self.plex_partner_id = plex_partner_id
    ...
    def plexca_csv_to_data(self):
        ...
        list_confluence_channels = [{"start_date":"01/01/1970 12:00:01 AM","end_date":"12/31/2099 11:59:59 PM", "linear_channel_nr": None, "packaged_service_id": None, "channelName":entry[1],"call_sign": entry[4],"channel_description":entry[5],"stationId":"epgProvider:st."+ str(entry[3]),"playbackUrlChannelId":entry[6],"channelChangeId":entry[7]} for entry in channel_data]
    ...
    def open_mso_file_data(self):
        ...
        self.validation_data = [{"channelName":channel_st_id[i],"stationId":prtn_st_id[i],"packaged_service_id":self.pckd_serv_id[i],"start_date":start_date[i],"end_date":end_date[i]} for i in range(len(prtn_st_id))]
    ...
    def write_channel_page(self):
        ...
        for entry in self.list_confluence_channels_stripped:
            ...
            channel_sheet.cell(row=incremental_row, column=11).value = f"tivo:pt.{self.plex_partner_id}"
    ...
    def write_msoSiteID_pages(self):
        ...
        for data in range(len(page_columns)):
            ...
            for entry in self.list_confluence_channels_stripped:
                ...
                channel_sheet.cell(row=incremental_row, column=9).value = entry["channelChangeId"]
                channel_sheet.cell(row=incremental_row, column=10).value = entry["playbackUrlChannelId"]
    ...
    def save_file(self):
        try:
            self.excel_file_path = f"{self.folder}/Plex_CA_{self.file_name}_file.xlsx"
