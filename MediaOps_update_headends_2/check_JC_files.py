#!/usr/bin/env python3
"""
Updated process_device_types.py

Change:
- When checking a headend's configuration, there can be multiple
  <configuration>/<applicableDeviceType> elements. If ANY of them
  equals "stb" (case-insensitive) then isSTB = "Y". Otherwise isSTB = "N".
- Preserves previous fallback: if there are no applicableDeviceType
  values found for that headend, treat as isSTB = "Y" (keeps original behavior).
"""
...
def fetch_applicable_types_and_stb_flag(host: str, partner_id: str, headend_id: str) -> Tuple[Optional[List[str]], bool]:
    """
    Fetch all applicableDeviceType texts for a given partner/headend.
    Returns (list_of_applicable_types_or_None, stb_present_bool)

    Logic:
      - Parse the mind39 response and collect all <applicableDeviceType> element texts.
      - If any of them equals "stb" (case-insensitive) then stb_present_bool = True.
      - If no applicableDeviceType elements found, returned list is None and stb_present_bool is False.
    """
    url = f"http://{host}.tivo.com:8085/mind/mind39"
    params = {
        "type": "serviceConfigurationSearch",
    ...
    types_set = find_all_texts_for_tag(text, "applicabledevicetype")
    if not types_set:
        return None, False

    types_list = sorted(types_set)
    stb_present = any(t.strip().lower() == "stb" for t in types_set)
    return types_list, stb_present
    ...
    for pid in partner_ids:
        ...
        with open(csv_filename, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for hid in headend_ids:
                applicable_list, stb_present = fetch_applicable_types_and_stb_flag(host, pid, hid)
                print(f"  headend {hid}: applicableDeviceTypes={applicable_list!r}, stb_present={stb_present!r}")
                # New logic:
                # - If no applicableDeviceType values found -> keep original behavior: isSTB = "Y"
                # - If any applicableDeviceType == "stb" -> isSTB = "Y"
                # - Otherwise -> isSTB = "N"
                if (not applicable_list) or stb_present:
                    isSTB = "Y"
                else:
                    isSTB = "N"
